export const PlaygroundCard = ({
    ...
    return (
        <div className="w-full max-w-lg space-y-4 cursor-none py-3 card" onClick={() => window.open(link, "_blank")}>
            ...
            <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                    {tags.map((tag, index) => (
                        <span
                            key={index}
                            className="inline-block text-xs font-medium px-4 py-2 bg-transparent text-gray-800 border rounded-sm border-zinc-400 "
                        >
                            {tag}
                        </span>
                    ))}
